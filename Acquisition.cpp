        ...
        for (unsigned int imageCnt = 0; imageCnt < k_numImages; imageCnt++)
        {
            try
            {
                ...
                else
                {
                    ...
                    const size_t channel = pResultImage->GetNumChannels();

                    cout << "Grabbed image " << imageCnt << ", width = " << width << ", height = " << height << ", channel = " << channel << endl;
                    ...
                    // optional parameter.
                    //
                    ImagePtr convertedImage = processor.Convert(pResultImage, Spinnaker::PixelFormat_RGB8);

                    //
                    // Convert image to RGB OpenCV Mat
                    //
                    // *** NOTES ***
                    // Since the channel order of the OpenCV color image is BGR,
                    // and the channel order of the image before conversion is RGB,
                    // it is necessary to perform channel conversion on the converted OpenCV Mat.
                    //
                    cv::Mat frame;
                    frame = cv::Mat(pResultImage->GetHeight() + pResultImage->GetYPadding(),
                                    pResultImage->GetWidth() + pResultImage->GetXPadding(),
                                    CV_8UC3);
